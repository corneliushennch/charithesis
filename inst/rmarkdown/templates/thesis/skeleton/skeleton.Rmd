...
title: 'Dissertation title in english'
gertitle: 'Das ist der deutsche Titel' # The german title is optional.
author: 'Vorname Nachname'
birthplace: 'Berlin'
date: '_______________' # This should be left free until the thesis is handed in.
institution: "Medizinischen Fakultät Charité – Universitätsmedizin Berlin"
department: 'Klinik/Institut für ...'
advisor: 'Prof. Dr. First Advisor'
altadvisor: 'Dr. med. Second Advisor' # If you have more two advisors, un-silence this line.
degree: 'Doctor medicinae (Dr. med.)'

# Rendering Options -----------------------------------------------------------
keep_tex: TRUE
knit: bookdown::render_book
site: bookdown::bookdown_site
# this sets a variety of options within the Latex template that is
# used to generate the PDF
header-includes:
# This allows you to change the spacing in your thesis. You can 
# switch out \onehalfspacing with \singlespacing or \doublespacing, if desired.
    - \usepackage{setspace}\singlespacing
    # This changes the font of the whole document. Other fonts are available like times, bookman, charter, palatino.
    - \usepackage{times}
    
# This will automatically install the {remotes} package and {charithesis}
# Change this to FALSE if you'd like to install them manually on your own.
params:
  'Install needed packages for {charithesis}': True

# Remove the hashtag to specify which version of output you would like.
# Can only choose one at a time.
output:
  charithesis::thesis_pdf: default 
#  charithesis::thesis_gitbook: default         
#  charithesis::thesis_word: default
#  charithesis::thesis_epub: default
...
acknowledgements: |
  I want to thank a few people. Most of all my wonderful girlfriend.
dedication: |
  You can have a dedication here if you wish. I dedicate this thesis to my grandma and my best friendt.
preface: |
  This is an example of a thesis setup to use the reed thesis document class 
  (for LaTeX) and the R bookdown package, in general. If you'd prefer to not include a Dedication, for example, simply delete the section entirely, or silence them (add # before each line). 
...
# This chunk ensures that the charithesis package is
# installed and loaded. This charithesis package includes
# the template files for the thesis.
if (!require(remotes)) {
  if (params$`Install needed packages for {charithesis}`) {
...
if (!require(charithesis)) {
  if (params$`Install needed packages for {charithesis}`) {
    remotes::install_github("corneliushennch/charithesis")
  } else {
    stop(
      paste(
        "You need to run",
